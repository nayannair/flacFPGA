...
def testbench_complete(samples, num_samples, order=4, block_size=16):
    if (num_samples <= order):
        bitstream = ''.join(binify(samples[0:num_samples]))
        return bitstream
    elif (num_samples <= block_size):
        warm_up = testbench_complete(samples, order, order, block_size)
        min_enc_resids = None
        rice_param_select = -1
        for rice_param in range(0, 15):
            enc_resids = get_enc_residuals(get_residuals(samples, num_samples, 0, order), num_samples-order, rice_param)
            if (min_enc_resids is None or len(enc_resids) <= len(min_enc_resids)):
                min_enc_resids = enc_resids
                rice_param_select = rice_param
        rice_param_rep = dec2bin(rice_param_select, 4)
        return warm_up + rice_param_rep + min_enc_resids
    else:
        remaining_samples = num_samples % block_size
        num_blocks = num_samples // block_size
        bitstream_list = [''] * (num_blocks + (1 if (remaining_samples != 0) else 0))
        offset = 0
        ind = 0
        while ind < num_blocks:
            bitstream_list[ind] = testbench_complete(samples[offset : offset + block_size], block_size, order, block_size)
            ind += 1
            offset += block_size
        if (remaining_samples != 0):
            bitstream_list[-1] = testbench_complete(samples[num_samples - remaining_samples : num_samples], remaining_samples, order, block_size)
        return ''.join(bitstream_list)

def verify(filename, rice_param):
    with open(filename, 'r') as fin:
        correct = 0
        print("Rice parameter: {0}".format(rice_param))
        for num_samples in range(1, 37):
            vout = fin.readline().strip()
            bitstream = testbench(samples, num_samples, rice_param)
            sout = hexify(make_reg_ready(bitstream))
            if sout != vout:
                print("# samples: {0} | FAILURE".format(num_samples))
            else:
                print("# samples: {0} | SUCCESS".format(num_samples))
                correct += 1
        print("Accuracy: {0}/36".format(correct))

def verify_complete(filename):
    with open(filename, 'r') as fin:
        correct = 0
        for num_samples in range(1, 37):
            vout = fin.readline().strip()
            bitstream = testbench_complete(samples, num_samples)
            sout = hexify(make_reg_ready(bitstream))
            if sout != vout:
                print("# samples: {0} | FAILURE".format(num_samples))
            else:
